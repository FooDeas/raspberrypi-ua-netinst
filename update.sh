...
check_key() {
	...
	if [ ! "$(cat "${KEY_FILE}" | gpg --homedir gnupg --keyid-format long --with-fingerprint --with-colons | grep -c "^pub:")" -eq 1 ]; then
		echo "FAILED!"
	...
	if [ "$(cat "${KEY_FILE}" | gpg --homedir gnupg --keyid-format long --with-fingerprint --with-colons | grep ^fpr: | awk -F: '{print $10}')" != "${KEY_FINGERPRINT}" ]; then
		echo "FAILED!"
